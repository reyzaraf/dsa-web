...
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h4 class="card-title">
                            <a href="{{ route('admin.categories.create') }}"class="add-menu-sidebar" >Create New</a>
                        </h4>
                    </div>
                    <div class="card-body">
                            ...
                            <table id="datatable" class="display min-w850">
                                ...
                                <tbody>
                                    @foreach ($categories as $category)
                                        <tr data-identifier="{{ $category->id }}">
                                            <td></td>
                                            <td>{{ $category->name }}</td>
                                            ...
                                            <form action="{{ route('admin.categories.destroy', $category->id) }}" method="post"
                                                id="destroy-{{ $category->id }}" style="display: none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </tr>
    ...
    <form action="{{ route('admin.categories.bulkDelete') }}" id="deleteSelectedForm" method="POST" class="d-none">
        @csrf
        @method('DELETE')
